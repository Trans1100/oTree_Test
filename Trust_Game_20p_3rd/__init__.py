...
class Constants(BaseConstants):
    name_in_url = 'Trust_Game_20p_3rd'
    players_per_group = 2
    num_rounds = 2
    endowment = cu(10)
...
def creating_session(subsession: Subsession):
    if subsession.round_number == 1:
        ...
        new_structure = [[6, 1], [2, 9], [4, 11], [19, 3], [8, 20],
                         [10, 13], [12, 14], [5, 18], [15, 7], [16, 17]]
        subsession.set_group_matrix(new_structure)
        subsession.get_group_matrix()
    if subsession.round_number == 2:
        subsession.get_group_matrix()
        new_structure = [[1, 6], [9, 2], [11, 4], [3, 19], [20, 8],
                         [13, 10], [14, 12], [18, 5], [7, 15], [17, 16]]
        subsession.set_group_matrix(new_structure)
        subsession.get_group_matrix()
...
class SendBack(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        player.participant.vars['round3' + str(player.participant.id_in_session) + 'sent_amount'] =\
            player.group.sent_amount
        player.participant.vars['round3' + str(player.participant.id_in_session) + 'sent_back_amount'] =\
...
class ResultsSummary(Page):
    ...
    def vars_for_template(player: Player):
        ...
        player.participant.vars['round3' + str(player.participant.id_in_session) + 'trust_p'] =\
            player.final_payoff
        return {
