...
def creating_group(subsession: Subsession):
    ...
    for p in subsession.get_players():
        ...
        if p.rank in [5, 8, 11, 16]:
            group_1.append(p)
        elif p.rank in [4, 7, 13, 19]:
            group_2.append(p)
        elif p.rank in [1, 10, 14, 20]:
            group_3.append(p)
        elif p.rank in [2, 6, 12, 17]:
            group_4.append(p)
        else:
            group_5.append(p)
    ...
    for player in group.get_players():
        for other_player in player.get_others_in_group():
            if other_player.participant.id_in_session ==\
                    player.participant.vars['round3' + str(player.participant.id_in_session) + 'id'][0]:
                other_player.ranking += player.choice_A
            elif other_player.participant.id_in_session ==\
                    player.participant.vars['round3' + str(player.participant.id_in_session) + 'id'][1]:
...
#        other_player_trust = []
        ...
        for other_player in player.get_others_in_group():
            other_player_sent.append(other_player.participant.vars
                                     ['round3' + str(other_player.participant.id_in_session) + 'sent_amount'])
            other_player_sent_back.append(other_player.participant.vars
                                          ['round3' + str(other_player.participant.id_in_session) + 'sent_back_amount'])
...
#                                      ['round2' + str(other_player.participant.id_in_session)+'t'])
        ...
        player.participant.vars['round3' + str(player.participant.id_in_session) + 'id'] =\
            other_player_id
...
class Result(Page):
    ...
    @staticmethod
    def vars_for_template(player: Player):
        player.participant.vars['round3' + str(player.participant.id_in_session) + 'leader_p'] = player.payoff
        return {
