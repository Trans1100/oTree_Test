...
class Constants(BaseConstants):
    name_in_url = 'Trust_Game_20p_2nd'
    players_per_group = 2
    num_rounds = 2
    endowment = cu(10)
...
def creating_session(subsession: Subsession):
    if subsession.round_number == 1:
        ...
        new_structure = [[1, 8], [2, 15], [3, 17], [4, 5], [7, 16],
                         [10, 12], [6, 14], [13, 18], [11, 9], [20, 19]]
        subsession.set_group_matrix(new_structure)
        subsession.get_group_matrix()
    if subsession.round_number == 2:
        subsession.get_group_matrix()
        new_structure = [[8, 1], [15, 2], [17, 3], [5, 4], [16, 7],
                         [12, 10], [14, 6], [18, 13], [9, 11], [19, 20]]
        subsession.set_group_matrix(new_structure)
        subsession.get_group_matrix()
...
class SendBack(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        ...
        player.participant.vars['round2' + str(player.participant.id_in_session) + 'sent_amount'] =\
            player.group.sent_amount
        player.participant.vars['round2' + str(player.participant.id_in_session) + 'sent_back_amount'] =\
...
class ResultsSummary(Page):
    ...
    def vars_for_template(player: Player):
        ...
        player.participant.vars['round2' + str(player.participant.id_in_session) + 'trust_p'] =\
            player.final_payoff
        return {
