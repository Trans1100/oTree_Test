...
class Constants(BaseConstants):
    name_in_url = 'Trust_Game_20p_1st'
    players_per_group = 2
    num_rounds = 2
    endowment = cu(10)
...
def creating_session(subsession: Subsession):
    if subsession.round_number == 1:
        ...
        new_structure = [[1, 20], [2, 19], [3, 18], [4, 17], [5, 16],
                         [6, 15], [7, 14], [8, 13], [9, 12], [10, 11]]
        subsession.set_group_matrix(new_structure)
        subsession.get_group_matrix()
    if subsession.round_number == 2:
        subsession.get_group_matrix()
        new_structure = [[20, 1], [19, 2], [18, 3], [17, 4], [16, 5],
...
class ResultsSummary(Page):
    ...
    def vars_for_template(player: Player):
        ...
        player.participant.vars['round1' + str(player.participant.id_in_session) + 'trust_p'] =\
            player.final_payoff
        return {
